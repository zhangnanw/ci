...
public class TableScanSikpRC {
	 ...
	 *
	 * @param arr2 被扫描的二维数组
	 * @param scanRow true 按X轴扫描，false按Y轴扫描。
	 *
	 * @return
	 */
	...
	static String[][] _toArr2(Element tableElement, PrintWriter returnOut) {
		...
		for (int rowIdx = 0; rowIdx < td2List.size(); rowIdx++) {
			...
			for (Element element : td2List.get(rowIdx)) {
				...
				int col = 1;
				int row = 1;
				if (StringUtils.isNumeric(colStr)) col = Integer.parseInt(colStr);
				if (StringUtils.isNumeric(rowStr)) row = Integer.parseInt(rowStr);
