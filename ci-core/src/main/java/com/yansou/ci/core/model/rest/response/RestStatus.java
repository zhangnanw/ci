...
public enum RestStatus {
	 ...
	 * <li>HEAD: the entity-header fields corresponding to the requested resource are sent in the response without any
	 * message-body;</li>
	 ...
	 * resource before returning the 201 status code. If the action cannot be carried out immediately, the server
	 * SHOULD
	 ...
	 * server,
	 * but is gathered from a local or a third-party copy. The set presented MAY be a subset or super set of the
	 * original version. For example, including local annotation information about the resource might result in a super
	 * set of the meta information known by the origin server. Use of this response code is not required and is only
	 ...
	 * updated
	 * meta information. The response MAY include new or updated meta information in the form of entity-headers, which
	 ...
	 * the user agent's active document view, although any new or updated meta information SHOULD be applied to the
	 ...
	 * to be sent. This response is primarily intended to allow input for actions to take place via user input,
	 * followed
	 ...
	 * field (section 14.35) indicating the desired range, and MAY have included an If-Range header field (section
	 * 14.27) to make the request conditional.
	 * <p/>
	 * <p>
	 * The response MUST include the following header fields:
	 * <ul>
	 * <li>Either a Content-Range header field (section 14.16) indicating the range included with this response, or a
	 ...
	 * <li>Expires, Cache-Control, and/or Vary, if the field-value might differ from that sent in any previous response
	 * for the same variant</li>
	 * </ul>
	 * <p/>
	 * <p>
	 ...
	 */
	PARTIAL_CONTENT(206), /**
	 * The 207 (Multi-Status) status code provides status for multiple independent operations (see Section 13 for more
	 * information).
	 * <p/>
	 * <p>
	 * A Multi-Status response conveys information about multiple resources in situations where multiple status codes
	 * might be appropriate. The default Multi-Status response body is a text/xml or application/xml HTTP entity with a
	 * 'multistatus' root element. Further elements contain 200, 300, 400, and 500 series status codes generated during
	 * the method invocation. 100 series status codes SHOULD NOT be recorded in a 'response' XML element.
	 * <p/>
	 * <p>
	 * Although '207' is used as the overall response status code, the recipient needs to consult the contents of the
	 * multistatus response body for further information about the success or failure of the method execution. The
	 * response MAY be used in success, partial success and also in failure situations.
	 * <p/>
	 * <p>
	 * The 'multistatus' root element holds zero or more 'response' elements in any order, each with information about
	 * an individual resource. Each 'response' element MUST have an 'href' element to identify the resource.
	 */
	MULTI_STATUS(207), /**
	 ...
	 * use one of the returned URIs. Clients with link editing capabilities ought to automatically re-link
	 * references to
	 ...
	 * The response MUST include the following header fields:
	 * <ul>
	 * <li>Date, unless its omission is required by section 14.18.1 If a clockless origin server obeys these rules, and
	 * proxies and clients add their own Date to any response received without one (as already specified by [RFC 2068],
	 * section 14.19), caches will operate correctly.</li>
	 * <li>ETag and/or Content-Location, if the header would have been sent in a 200 response to the same request</li>
	 * <li>Expires, Cache-Control, and/or Vary, if the field-value might differ from that sent in any previous response
	 * for the same variant</li>
	 * </ul>
	 ...
	 * status code is commonly used when the server does not wish to reveal exactly why the request has been
	 * refused, or
	 ...
	 * header field (section 14.34). HTTP access authentication is explained in
	 * "HTTP Authentication: Basic and Digest Access Authentication" [43].
	 */
	 ...
	 * request. The response body SHOULD include enough information for the user to recognize the source of the
	 ...
	 * Request-URI after user approval. If the server does not know, or has no facility to determine, whether or not
	 * the
	 * condition is permanent, the status code 404 (Not Found) SHOULD be used instead. This response is cacheable
	 * unless
	 * indicated otherwise.
	 * <p/>
	 * <p>
	 ...
	 * interpret. This rare condition is only likely to occur when a client has improperly converted a POST request
	 * to a
	 * GET request with long query information, when the client has descended into a URI "black hole" of redirection
	 * (e.g., a redirected URI prefix that points to a suffix of itself), or when the server is under attack by a
	 * client
	 ...
	 * resource, and the request did not include an If-Range request-header field. (For byte-ranges, this means that
	 * the
	 ...
	 * SHOULD
	 * contain an appropriate precondition or postcondition code, such as 'lock-token-submitted' or
	 * 'no-conflicting-lock'.
	 */
	LOCKED(423), /**
