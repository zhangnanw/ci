package org.yansou.ci.core.rest.response.project;

import org.yansou.ci.common.page.Pagination;
import org.yansou.ci.core.db.model.project.PlanBuildData;
import org.yansou.ci.core.rest.response.RestResponse;

public class PlanBuildDataPaginationResponse extends RestResponse<Pagination<PlanBuildData>> {

	private static final long serialVersionUID = -7923982956522231678L;

}
